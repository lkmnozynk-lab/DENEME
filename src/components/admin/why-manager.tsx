...
const initial: WhyState = { ok: false, message: "" };

function IconSelect({ name, defaultValue }: { name: string; defaultValue?: string }) {
  return (
    <select
      name={name}
      defaultValue={defaultValue ?? "sparkle"}
...
export function WhyManager({ items }: { items: Item[] }) {
  const [state, addAction] = useActionState(createWhyReason, initial);

  return (
    <Card className="space-y-5">
      <div>
        <h2 className="font-semibold">Neden NERAAJANS</h2>
        <p className="mt-1 text-sm text-muted">
          Ana sayfadaki “Neden NERAAJANS” bölümünde görünen sebepler.
        </p>
      </div>
        ...
        {items.map((it) => (
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <form action={updateWhyReason} className="flex-1 space-y-2">
              ...
              <DeleteButton action={deleteWhyReason} id={it.id} iconOnly />
            </div>
          </div>
        ))}
      </div>
      ...
      <form action={addAction} className="space-y-3 border-t border-border pt-5">
        <p className="text-sm font-medium">Yeni Sebep Ekle</p>
